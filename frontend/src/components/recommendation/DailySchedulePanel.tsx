...
//   - Detects metadata.streaming flag from backend
//   - Shows spinner + "Planning day N of M..." while streaming
//   - Day pills appear progressively as each day arrives
//   - Nuggets grid hidden until streaming completes
//   - Smooth fadeIn animation on new day pills
//
...
// ─── Nugget styling config ───────────────────────────────────────────────

const NUGGET_STYLES: Record<string, { bg: string; border: string; titleColor: string; textColor: string }> = {
...
const NUGGET_ICONS: Record<string, string> = {
  packing_tips: '🧳',
  local_events: '🎪',
  cuisine_rationale: '🍽️',
  activity_rationale: '🎭',
...
// ─── Legacy text parser (text_v8 fallback) ───────────────────────────────
...
function parseLegacyBlocks(planText: string): Array<{ title: string; body: string }> {
  ...
  if (hasHash) {
    for (const section of hashSections) {
      ...
      if (nl === -1) {
        blocks.push({ title: trimmed.replace(/^#+\s*/, ''), body: '' });
      } else {
        blocks.push({
          title: trimmed.slice(0, nl).replace(/^#+\s*/, '').trim(),
    ...
    for (const section of boldSections) {
      ...
      if (match) {
        blocks.push({ title: match[1].trim(), body: trimmed.slice(match[0].length).trim() });
      } else {
        const colon = trimmed.indexOf(':');
        if (colon > 0 && colon < 80) {
          blocks.push({ title: trimmed.slice(0, colon).replace(/\*\*/g, ''), body: trimmed.slice(colon + 1).trim() });
        } else {
          blocks.push({ title: 'Schedule', body: trimmed });
        }
      }
    }
  }
...
export const DailySchedulePanel: React.FC<Props> = ({ dailyPlanRec, destination }) => {
  const [expandedDay, setExpandedDay] = useState(0);

  // ── Auto-select the latest day as it streams in ──────────────────────
  const daysComplete = dailyPlanRec?.metadata?.days_complete || 0;
  const isStreaming = dailyPlanRec?.metadata?.streaming === true;

  useEffect(() => {
    // When a new day arrives during streaming, auto-select it
    if (isStreaming && daysComplete > 0) {
      setExpandedDay(daysComplete - 1);
    }
  ...
  if (isStructured) {
    if (dailyPlanRec.metadata?.structured_data) {
      ...
      try {
        structuredPlan = JSON.parse(dailyPlanRec.reason) as StructuredPlan;
      } catch {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // LEGACY RENDERING (text_v8 or parse failure)
  // ═══════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // STRUCTURED RENDERING (structured_v1)
  // ═══════════════════════════════════════════════════════════════════════
  ...
  return (
    <div>
      ...
      <style>{`
        ...
        @keyframes dsp-fadeSlideUp {
          from { opacity: 0; transform: translateY(12px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes dsp-pillFadeIn {
          from { opacity: 0; transform: scale(0.85); }
          to { opacity: 1; transform: scale(1); }
        }
        @keyframes dsp-pulseGlow {
          0%, 100% { opacity: 0.6; }
          50% { opacity: 1; }
        }
      `}</style>
      ...
      <div className="flex items-center gap-1.5 mb-2.5">
        ...
        <h4
          className="text-[13px] font-extrabold text-gray-800"
      ...
      {/* ── Day selector pills (COMPACT) + streaming placeholder pills ──── */}
      <div className="flex gap-1.5 mb-3 overflow-x-auto pb-0.5">
        ...
        {daily_schedule.map((day, i) => {
          ...
          return (
            <button
              ...
              className="flex-1 min-w-[52px] rounded-lg border-none cursor-pointer transition-all duration-300"
              style={{
                padding: '6px 4px',
                background: isActive ? t.gradient : 'white',
                border: isActive ? 'none' : '1.5px solid #F1F5F9',
                boxShadow: isActive ? '0 3px 12px rgba(0,0,0,0.15)' : 'none',
            ...
            >
              <div className="text-center">
                ...
                <div style={{
                  ...
                  marginTop: 1,
                }}>{formatDayDate(day.date)}</div>
                {day.weather && (
                  <>
                    <div style={{ fontSize: 12, marginTop: 2 }}>{day.weather.icon}</div>
                    <div style={{
                      fontSize: 8, fontWeight: 600,
                      color: isActive ? 'rgba(255,255,255,0.9)' : '#64748B',
                      marginTop: 1,
                    }}>{Math.round(day.weather.temp_high)}°F</div>
                  </>
                )}
              </div>
            </button>
        ...
        {/* Placeholder pills for days not yet streamed */}
        {isStreaming && Array.from({ length: daysTotal - daily_schedule.length }, (_, i) => {
          ...
          return (
            <div
              ...
              className="flex-1 min-w-[52px] rounded-lg"
              style={{
                padding: '6px 4px',
                ...
                <div style={{ marginTop: 4 }}>
                  <div
                    ...
                    style={{
                      ...
                      animation: dayNum === daily_schedule.length + 1
                        ? 'dsp-pulseGlow 1.2s ease-in-out infinite'
                        : 'none',
                    }}
                  />
                </div>
      ...
      {isStreaming && (
        <div className="flex items-center gap-2 mb-3 px-1">
          ...
          <span
            className="journal-text text-purple-600"
            style={{ fontSize: 19 }}
          >
      ...
      {currentDay && (
        <div
          ...
          style={{
            ...
            animation: 'dsp-fadeSlideUp 0.35s ease forwards',
          }}
        >
          {/* Day header with gradient (COMPACT) */}
          <div
          ...
          >
            <div>
              ...
              <h3
                className="text-[15px] font-extrabold text-white mt-0.5"
                style={{ fontFamily: "'Plus Jakarta Sans', 'DM Sans', sans-serif" }}
              >
                {currentDay.title}
              </h3>
              ...
              >
                ...
                <div>
                  <div className="text-xs font-bold text-white">
                    {Math.round(currentDay.weather.temp_high)}°F
                  </div>
          ...
          {/* Journal intro (LARGER FONT) */}
          {currentDay.intro && (
            <div className="px-4 pt-3 pb-2" style={{ borderBottom: `1px dashed ${theme.color}15` }}>
              <p
                className="journal-text leading-relaxed"
                style={{ fontSize: '21px', color: '#4A3728', lineHeight: 1.45 }}
              >
                ✏️ {currentDay.intro}
              </p>
            </div>
          ...
          {/* Timeline slots (LARGER HANDWRITTEN FONT) */}
          <div className="px-4 py-2">
            {currentDay.slots.map((slot, si) => {
              ...
              return (
                <div
                  key={si}
                  className="flex gap-3.5 py-2.5 rounded-xl transition-colors"
                  style={{
                    borderBottom: si < currentDay.slots.length - 1
                ...
                >
                  ...
                  <div className="flex flex-col items-center flex-shrink-0" style={{ width: 44 }}>
                    ...
                    <div
                      className="flex items-center justify-center mt-1 rounded-lg"
                      style={{
                    ...
                    {si < currentDay.slots.length - 1 && (
                      <div
                        className="flex-1 mt-1 rounded"
                        style={{
                          width: 2, minHeight: 16,
                          background: `linear-gradient(to bottom, ${theme.color}25, transparent)`,
                        }}
                      />
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    {slot.narrative && (
                      <p
                        className="journal-text mt-1 leading-relaxed"
                        style={{ fontSize: '21px', color: '#5D4E37', lineHeight: 1.4 }}
                      >
                        {slot.narrative}
                      </p>
                    )}
                    ...
                    {slot.google_url && (
                      <a
                        href={slot.google_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 mt-1 text-[9px] font-medium text-gray-400 hover:text-purple-500 transition-colors"
                      >
                        📍 View on Google Maps
                      </a>
                    )}
          ...
          {/* Day summary strip (COMPACT) */}
          <div
            className="flex items-center gap-3 px-4 py-2 text-[10px] font-semibold text-gray-500"
            style={{ background: `${theme.color}06`, borderTop: `1px solid ${theme.color}12` }}
          >
            ...
            {currentDay.weather && (
              <>
                <span className="w-px h-3 bg-gray-200" />
                <span>{currentDay.weather.icon} {Math.round(currentDay.weather.temp_high)}°F · {currentDay.weather.description}</span>
              </>
            )}
          </div>
        </div>
      ...
      {/* ── Info nuggets grid (hidden during streaming) ─────────────────── */}
      {nuggets && nuggets.length > 0 && !isStreaming && (
        <div
          className="grid grid-cols-2 gap-2 mt-3"
          style={{ animation: 'dsp-fadeSlideUp 0.4s ease forwards' }}
        >
          {nuggets.map((nugget, i) => {
            ...
            return (
              <div
                key={nugget.id}
                className={`${style.bg} border ${style.border} rounded-xl p-2.5 transition-all hover:-translate-y-0.5 hover:shadow-md cursor-default ${isWide ? 'col-span-2' : ''}`}
              >
                <div className="flex items-start gap-1.5">
                  <span className="text-sm flex-shrink-0">{icon}</span>
                  <div className="min-w-0">
                    <h5
                      className={`text-[10px] font-bold ${style.titleColor}`}
                      style={{ fontFamily: "'Plus Jakarta Sans', 'DM Sans', sans-serif" }}
                    >
                      {nugget.title}
                    </h5>
                    <p className={`text-[9px] ${style.textColor} leading-snug mt-0.5`}>
                      {nugget.content}
                    </p>
                  </div>
                </div>
