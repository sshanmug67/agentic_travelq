        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-[13px] font-medium">
                {flight.cabin_class}
              </span>
          ...
          <div className="text-right">
            <span className="text-[19px] font-bold text-green-600">${flight.price}</span>
            <span className="text-[13px] text-gray-400 ml-1">pp</span>
          </div>
        </div>

        {/* Outbound row */}
        <div className="flex items-center gap-2 py-1.5 px-2 bg-blue-50/60 rounded text-[15px]">
          <span className="text-[13px] font-semibold text-blue-600 w-8 flex-shrink-0">OUT</span>
          <span className="font-semibold text-gray-800 w-10">{flight.outbound?.departure_airport}</span>
          <span className="text-[13px] text-gray-500">{formatTime(flight.outbound?.departure_time)}</span>
          <div className="flex-1 flex items-center px-1">
            <div className="flex-1 h-px bg-blue-300" />
            <span className="text-[13px] text-gray-400 px-1.5">
              {flight.outbound?.duration}
            </span>
            <div className="flex-1 h-px bg-blue-300" />
          </div>
          <span className="font-semibold text-gray-800 w-10 text-right">{flight.outbound?.arrival_airport}</span>
          <span className="text-[13px] text-gray-500 w-16 text-right">{formatTime(flight.outbound?.arrival_time)}</span>
          <span className="text-[13px] text-gray-400 w-14 text-right">{formatDate(flight.outbound?.departure_time)}</span>
          {(flight.outbound?.stops ?? 0) > 0 && flight.outbound?.layovers && flight.outbound.layovers.length > 0 && (
            <span className="text-[13px] text-gray-400 ml-1">via {flight.outbound.layovers.join(',')}</span>
          )}
        </div>

        {/* Return row */}
        {flight.return_flight && (
          <div className="flex items-center gap-2 py-1.5 px-2 bg-purple-50/60 rounded text-[15px] mt-1">
            <span className="text-[13px] font-semibold text-purple-600 w-8 flex-shrink-0">RET</span>
            <span className="font-semibold text-gray-800 w-10">{flight.return_flight.departure_airport}</span>
            <span className="text-[13px] text-gray-500">{formatTime(flight.return_flight.departure_time)}</span>
            <div className="flex-1 flex items-center px-1">
            ...
            <span className="text-[13px] text-gray-500 w-16 text-right">{formatTime(flight.return_flight.arrival_time)}</span>
            <span className="text-[13px] text-gray-400 w-14 text-right">{formatDate(flight.return_flight.departure_time)}</span>
            {flight.return_flight.stops > 0 && flight.return_flight.layovers && flight.return_flight.layovers.length > 0 && (
              <span className="text-[13px] text-gray-400 ml-1">via {flight.return_flight.layovers.join(',')}</span>
            )}
          </div>
        )}
